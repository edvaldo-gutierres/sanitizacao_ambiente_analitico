...
def criar_tabelas_no_dbthanos(df: pd.DataFrame, target_db: str) -> None:
    ...
    for index, row in df.iterrows():
        print(f'Processando criação da Tabela no [dbthanos]: {row["database_name"]}.{row["schema_name"]}.{row['table_name']}')
        create_table_dbthanos(
...
def drop_tabelas_origem(df: pd.DataFrame) -> None:
    ...
    for index, row in df.iterrows():
        print(f'Processando exclusão da Tabela: {row['database_name']}.{row['schema_name']}.{row['table_name']}')
        drop_table_origin(
